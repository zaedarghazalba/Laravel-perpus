        ...
        <form action="{{ route('admin.books.update', $book) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="number" name="publication_year" id="publication_year" value="{{ old('publication_year', $book->publication_year) }}" min="1000" max="{{ date('Y') + 1 }}"
                ...
                <div>
                    ...
                    <div class="flex gap-2 mt-1">
                        <input type="text" name="barcode" id="barcode" value="{{ old('barcode', $book->barcode) }}"
                ...
                <div class="relative">
                    ...
                    <input type="text" id="classification_search"
                        ...
                        autocomplete="off">
                    <input type="hidden" name="classification_code" id="classification_code" value="{{ old('classification_code', $book->classification_code) }}">
                    ...
                    <div id="selected_classification" class="{{ $book->classification_code ? '' : 'hidden' }} mt-2 p-3 bg-blue-50 border border-blue-200 rounded-md">
                        <div class="flex items-center justify-between">
                            <div>
                                <span class="font-semibold text-blue-900" id="selected_code">{{ $book->classification_code }}</span>
                                <span class="text-sm text-blue-700 ml-2" id="selected_name">{{ $book->classification->name ?? '' }}</span>
                            </div>
                ...
                <div>
                    ...
                    <input type="text" name="call_number" id="call_number" value="{{ old('call_number', $book->call_number) }}" readonly
                ...
                <div>
                    ...
                    <input type="text" name="shelf_location" id="shelf_location" value="{{ old('shelf_location', $book->shelf_location) }}" readonly
                        class="mt-1 block w-full rounded-md border-gray-300 bg-gray-50 shadow-sm"
                        placeholder="Auto-generate dari klasifikasi">
                ...
                <div>
                    ...
                    <input type="number" name="quantity" id="quantity" value="{{ old('quantity', $book->quantity) }}" required min="0"
                    ...
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">
                        Stok saat ini: {{ $book->quantity }} | Tersedia: {{ $book->available_quantity }} | Dipinjam: {{ $book->quantity - $book->available_quantity }}
                    </p>
                </div>
                ...
                <div>
                    ...
                    @if($book->cover_image)
                        <div class="mt-2 mb-3">
                            <img src="{{ asset($book->cover_image) }}" alt="{{ $book->title }}" class="h-32 w-24 object-cover rounded shadow">
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" id="description" rows="4"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $book->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
