...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Tambah Klasifikasi Baru</h2>
            <p class="mt-1 text-sm text-gray-600">Tambahkan klasifikasi DDC baru ke sistem</p>
        </div>
            ...
            <form action="{{ route('admin.classifications.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="4"
                              ...
                              style="focus:ring-color: #007FFF;">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <select name="level" id="level" required
                        ...
                        <option value="1" {{ old('level') == '1' ? 'selected' : '' }}>Level 1 - Kelas Utama (000, 100, 200, ...)</option>
                        <option value="2" {{ old('level') == '2' ? 'selected' : '' }}>Level 2 - Divisi (010, 020, 030, ...)</option>
                        <option value="3" {{ old('level') == '3' ? 'selected' : '' }}>Level 3 - Seksi (005, 015, 025, ...)</option>
                        <option value="4" {{ old('level') == '4' ? 'selected' : '' }}>Level 4 - Spesifik (005.1, 005.2, ...)</option>
                    </select>
                    @error('level')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="parent_code" id="parent_code"
                        ...
                        @foreach($parentClassifications as $parent)
                            <option value="{{ $parent->code }}" {{ old('parent_code') == $parent->code ? 'selected' : '' }}>
